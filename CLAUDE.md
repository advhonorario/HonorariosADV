...
**Fase 1 (banco), Fase 2 (shell, login, cadastros) e Fase 3 (tela de lançamento com rateio e barra) concluídas.** Próxima fase pendente: Fase 4 (edição, estorno, pop-up de motivo, logs) — ver especificação §13. A tela de lançamento (`public/js/telas/lancamento.js`) cobre só a criação de lançamentos nesta fase; clique-para-editar, duplo-clique em célula, estorno e o pop-up de motivo (§7.6) ficam para a Fase 4.

- Banco: 13 migrations em `supabase/migrations/` (`001` a `008` da Fase 1; `009_rpcs_cadastros.sql`, `010_realtime.sql` e `011_rpcs_reativar.sql` da Fase 2; `012_lancamento_fase3.sql` e `013_fix_rateio_update_where.sql` da Fase 3), aplicadas no projeto Supabase real. Não há ambiente local com Docker configurado nesta máquina — aplicar mudanças com `supabase db push --db-url <POSTGRES_URL_NON_POOLING> --include-all --yes` (variáveis em `.env.local`, nunca commitado) e sempre rodar `supabase db advisors --db-url <...> --type all --level warn` depois, esperando "No issues found".
