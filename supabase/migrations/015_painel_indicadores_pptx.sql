-- Fase 5 (continuação) — reconciliação com `Contabilidade 2025.pptx` (§2, item
-- pendente P1): adiciona ao `rpc_painel` os indicadores que já existiam nos
-- slides do escritório e ainda não tinham equivalente no sistema:
--   • processos_por_ano          — nº de processos distribuídos, por ano
--   • variacao_anual             — escritório + top 5 advogados, ano a ano
--   • maiores_honorarios_advogado — top 10 clientes de UM advogado filtrado
--   • divisao_por_indicador      — total por advogado indicador (papel='indicacao')
--   • divisao_faixa_valor        — lançamentos por faixa de valor (>5k / 2-5k / <2k)
--
-- `processos_por_ano` não é filtrado por período (de/ate) nem por perfil —
-- é uma contagem operacional plurianual, sem valores monetários por pessoa,
-- mesmo raciocínio para não restringir por advogado. As demais reaproveitam
-- `lanc_filtrados`/`rateio_filtrado` (já com o escopo por perfil da migration
-- 014) ou aplicam o mesmo padrão NULL-safe de `v_eh_advogado`/`v_advogado_escopo`.
...
begin
  ...
  select jsonb_build_object(
    ...
    'series', jsonb_build_object(
      ...
      'divisao_faixa_valor', (
        select coalesce(jsonb_agg(jsonb_build_object(
          'faixa', x.faixa, 'valor', x.valor, 'quantidade', x.quantidade,
          'percentual', round((x.valor / nullif(sum(x.valor) over (), 0)) * 100, 1)
