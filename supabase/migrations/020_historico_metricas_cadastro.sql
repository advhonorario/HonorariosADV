-- Fora das fases planejadas na especificação — pedido do usuário 2026-09-18:
-- tarefa periódica que registra a quantidade total de advogados, clientes,
-- processos e lançamentos, pra alimentar um gráfico de evolução de cadastros
-- no painel.
--
-- Agendamento: de 7 em 7 dias, sempre reiniciando no dia 1 de cada mês —
-- dias 1, 8, 15, 22 e 29 de cada mês, às 04:00 de Brasília. Quando o dia 29
-- não existe no mês (ex. fevereiro), o pg_cron simplesmente não dispara
-- nesse dia, e o ciclo volta a contar do dia 1 do mês seguinte — é o
-- comportamento pedido, sem precisar de lógica extra. O pg_cron do Supabase
-- roda em UTC por padrão, e o Brasil não tem mais horário de verão desde
-- 2019, então 04:00 de Brasília = 07:00 UTC o ano inteiro.

create table if not exists public.historico_metricas_cadastro (
  id uuid primary key default gen_random_uuid(),
  data_snapshot date not null unique,
  total_advogados integer not null,
...
alter table public.historico_metricas_cadastro enable row level security;

-- Defesa em profundidade pra acesso direto via REST — o gráfico no painel
-- não passa por aqui, lê via `rpc_painel` (security definer, ignora RLS).
create policy historico_metricas_cadastro_select on public.historico_metricas_cadastro
  for select using (coalesce(private.perfil_atual()::text, '') in ('admin','financeiro','socio'));

-- Contagem total simples (count(*) da tabela inteira, sem filtrar
-- estornado/inativo) — é a "quantidade total" pedida, não um recorte de
-- período.
create or replace function public.fn_snapshot_metricas_cadastro()
returns void as $$
begin
  -- delete + insert em vez de `on conflict do update`: o caminho interno do
  -- `on conflict do update` passa pelo mesmo executor de UPDATE, e a
  -- extensão `safeupdate` do projeto exige WHERE explícito em UPDATE/DELETE
  -- (ver CLAUDE.md) — mais simples e seguro evitar o upsert.
  delete from public.historico_metricas_cadastro where data_snapshot = current_date;

  insert into public.historico_metricas_cadastro
    (data_snapshot, total_advogados, total_clientes, total_processos, total_lancamentos)
  values (
    current_date,
...
-- Só o pg_cron (role postgres) chama essa função — não é uma RPC de negócio
-- pra cliente nenhum invocar via `supabase.rpc(...)`.
revoke all on function public.fn_snapshot_metricas_cadastro() from public, anon, authenticated;
...
select cron.unschedule(jobid) from cron.job where jobname = 'snapshot_metricas_cadastro';
select cron.schedule(
  'snapshot_metricas_cadastro',
  '0 7 1,8,15,22,29 * *',
  $$ select public.fn_snapshot_metricas_cadastro(); $$
);

-- Adiciona `evolucao_cadastros` à série do painel — não filtrado por período
-- nem por perfil, mesmo raciocínio já usado em `processos_por_ano` (migration
-- 015): é contagem operacional, sem valor monetário por pessoa.
...
begin
  ...
  anos_escritorio as (
    select extract(year from l.data_competencia)::int as ano,
           sum(l.valor_bruto) as escritorio,
           sum(l.valor_base) - coalesce(sum(rat.repasse), 0) as retencao
    from public.lancamentos l
    left join (
      select lancamento_id, sum(valor) as repasse
      from public.lancamento_rateio
      group by lancamento_id
    ) rat on rat.lancamento_id = l.id
    where l.status <> 'estornado'
      and (v_clientes is null or l.cliente_id = any(v_clientes))
      and (v_tipos_processo is null or l.tipo_processo_id = any(v_tipos_processo))
      and (v_tipos_servico is null or l.tipo_servico_id = any(v_tipos_servico))
  ...
  select jsonb_build_object(
    ...
    'indicadores', jsonb_build_object(
      ...
      'ticket_medio_tipo_processo', jsonb_build_object(
        'valor', (select case when count(distinct tipo_processo_id) = 0 then 0
                    else round(sum(valor_bruto) / count(distinct tipo_processo_id), 2) end
                  from lanc_filtrados)
      ),
      'ticket_medio_cliente', jsonb_build_object(
        'valor', (select case when count(distinct cliente_id) = 0 then 0
                    else round(sum(valor_bruto) / count(distinct cliente_id), 2) end
                  from lanc_filtrados)
      ),
      'ticket_medio_advogado', jsonb_build_object(
        'valor', (
          select case when count(distinct advogado_id) = 0 then 0
            else round((select coalesce(sum(valor_bruto),0) from lanc_filtrados) / count(distinct advogado_id), 2)
          end
          from rateio_filtrado
        )
      ),
    ...
    'series', jsonb_build_object(
      ...
      'evolucao_cadastros', (
        select coalesce(jsonb_agg(jsonb_build_object(
          ...
          'advogados', h.total_advogados,
          'clientes', h.total_clientes,
          'processos', h.total_processos,
          'lancamentos', h.total_lancamentos
        ) order by h.data_snapshot), '[]'::jsonb)
        from public.historico_metricas_cadastro h
      ),

      'variacao_anual', (
        select coalesce(jsonb_agg(jsonb_build_object(
          'ano', e.ano, 'escritorio', e.escritorio, 'retencao', e.retencao,
          'advogados', (
