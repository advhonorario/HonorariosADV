-- Fix: `rpc_painel` (Fase 1) não restringia nada para o perfil `advogado` —
-- qualquer advogado logado via o painel via a tela da Fase 5 (public/js/telas/painel.js)
-- veria os indicadores e gráficos do escritório inteiro, quebrando a regra da
-- especificação §5.4 ("advogado só enxerga seus próprios números").
--
-- Decisão de produto (2026-09-16): o escopo do advogado é por LANÇAMENTO, não
-- por linha de rateio — ele vê os lançamentos onde aparece no rateio, com os
-- valores CHEIOS do lançamento (valor_bruto, valor_base, repasse total, etc.),
-- não só a fatia dele. Mesma semântica já usada na policy `lanc_leitura` de
-- `007_rls.sql`.
--
-- Cuidado NULL-safe (regra já documentada no CLAUDE.md, bug já cometido antes
-- nesta mesma RPC de outra forma): a checagem usa um booleano `v_eh_advogado`
-- separado do uuid `v_advogado_escopo`, nunca `v_advogado_escopo is null` como
-- sinal de "sem restrição" — um advogado sem `usuarios.advogado_id` vinculado
-- (estado de dados quebrado) precisa falhar FECHADO (não ver nada), não
-- FECHADO significa aqui "sem restrição aplicada por engano".
